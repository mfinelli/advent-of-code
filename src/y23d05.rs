...
/// let input = "";
/// assert_eq!(y23d05(input), 0);
/// ```
pub fn y23d05(input: &str) -> u64 {
    let mut locations = BinaryHeap::new();
    let mut seeds: Vec<u64> = Vec::new();

    let mut seed_to_soil: Vec<(Range<u64>,Range<u64>)>= Vec::new();
    let mut soil_to_fertilizer: Vec<(Range<u64>,Range<u64>)>= Vec::new();
    let mut fertilizer_to_water: Vec<(Range<u64>,Range<u64>)>= Vec::new();
    let mut water_to_light: Vec<(Range<u64>,Range<u64>)>= Vec::new();
    let mut light_to_temperature: Vec<(Range<u64>,Range<u64>)>= Vec::new();
    let mut temperature_to_humidity: Vec<(Range<u64>,Range<u64>)>= Vec::new();
    let mut humidity_to_location: Vec<(Range<u64>,Range<u64>)>= Vec::new();
...
/// TODO
fn map_value_to_range(val: u64, mappings: &Vec<(Range<u64>, Range<u64>)>) -> u64 {
    for (dest, src) in mappings {
        if src.contains(&val) {
            return dest.start + (val - src.start);
        }
    }
...
mod tests {
    ...
    fn the_solution() {
        ...
        assert_eq!(y23d05(&contents), 324724204);
    }
}
